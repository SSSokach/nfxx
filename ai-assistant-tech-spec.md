...
|---|--------|---------|------|------|------|---------|---------|
| 1 | 快捷提示气泡 | [1.2](#sec-1.2) / [2.1](#sec-2.1) | ⚠️ | N/A | 框架存在但内容被注释禁用 | `AIPanel.vue` 第312-317行 `quickBubbles` 数组全部注释，实际不显示 |  |
| 2 | 消息手动创建待办入口 | [2.2](#sec-2.2) | ⚠️ | `create-from-message` 接口存在 ✅ | ❌ 无入口 | 前端 `createTodoFromMessage()` 函数和 API 封装均已实现，但触发入口被注释禁用（`ChatArea.vue` 第44-50行内联气泡整段注释），且右键菜单无"创建待办"选项。用户无法从单条消息直接创建待办，只能通过候选待办扫描间接创建 |  |
| 3 | 工作报告生成 | [2.5](#sec-2.5) | ⚠️ | `/api/reports/work-report` ✅ | API封装有 ✅，快捷入口被注释 | **双问题**：① 前端快捷气泡入口被注释（`AIPanel.vue` 第314行）；② 更关键的是 AI 对话流程中缺少 `generate_work_report` 和 `generate_daily_digest` 的 function calling 工具注册（`glm_ai.py` 的 `build_registry()` 未注册这两个工具），导致用户通过AI对话请求工作报告时，LLM 无法调用正确工具生成报告，只能凭上下文编造。后端 `/api/reports/work-report` 本身已正确区分 `status=completed`（按 `completed_at` 过滤）和 `status=pending` 待办 |  |
...
| 5 | <span style="color:red">**🔴 待办点击跳转原消息**</span> | [2.2](#sec-2.2) | <span style="color:red">**🔴 新增需求**</span> | `source_id` 字段已存储 ✅ | ❌ 前端无跳转逻辑 | 待办列表中每条待办支持点击后跳转到对应的原始消息（群聊/私聊/邮件），`chat_todo_item` 和 `email_todo_item` 表的 `source_id` 字段已存储原始消息/邮件ID，前端需实现点击跳转定位 |  |
...
- **AI对话**：统一的自然语言交互入口，文件摘要、工作报告等功能通过对话完成（如"@文件名 总结一下"、"帮我生成本周工作报告"）。初始状态下输入栏上方纵向排列快捷提示气泡，点击自动填充指令；用户发送首条消息后气泡自动隐藏。提供"新对话"按钮，点击后清空聊天记录并重新展示提示气泡。
  > ⚠️ **[实现差异]** 快捷提示气泡功能：前端框架存在（`AIPanel.vue` 有 `quickBubbles` 数组和渲染逻辑），但气泡内容全部被注释禁用（第312-317行），实际不显示。
- **待办**：合并消息待办与邮件待办，统一展示待办列表。待办创建不再仅依赖主动扫描，还支持在收到消息时于**对应消息下方**内联弹出"是否创建待办：[待办内容摘要]"气泡。
  > ⚠️ **[实现差异]** 内联气泡入口被注释禁用（`ChatArea.vue` 第44-50行），当前实际创建方式为：待办 Tab 点击"AI扫描"按钮 → 生成候选待办列表 → 用户确认后转为正式待办。
...
> ⚠️ **[实现差异]** 前端框架存在但内容被注释禁用（`AIPanel.vue` 第312-317行 `quickBubbles` 数组全部注释），实际不显示。
...
> ⚠️ **[实现差异]** 消息下方内联气泡被注释禁用（`ChatArea.vue` 第44-50行整段注释），且右键菜单无"创建待办"入口。当前实际创建方式为：待办 Tab 点击"AI扫描"按钮 → 生成候选待办列表 → 用户确认后转为正式待办。
...
- <span style="color:red">**点击跳转**：点击待办项跳转到对应的原始消息（群聊/私聊）或邮件详情</span>
...
- **简约侧边栏（AIPanel）**：仅 2 个 Tab
  ...
  - **待办 Tab**：消息待办 + 邮件待办合并展示，仅刷新按钮，无扫描按钮
    > ⚠️ **[实现差异]** 实际待办 Tab 有"AI扫描"按钮，用于扫描消息和邮件生成候选待办。
- **聊天区增强（ChatArea）**：
  - 输入框旁设置智能回复按钮（💡），点击后弹出纵向排列的候选回复列表
  - 收到对方消息时，在**消息下方**内联弹出"是否创建待办：\[内容摘要]"气泡（创建/忽略）
    > ⚠️ **[实现差异]** 内联气泡被注释禁用（`ChatArea.vue` 第44-50行），无入口。
...
> ⚠️ **[实现差异]** 此流程为设计方案，实际内联气泡入口被注释禁用。当前实际流程为：点击待办 Tab "AI扫描"按钮 → 扫描消息/邮件 → 生成候选待办列表 → 用户确认后转为正式待办。
...
3. 收到对方消息后，在**消息下方**内联弹出"是否创建待办：\[内容摘要]"气泡（创建/忽略）
   > ⚠️ **[实现差异]** 此气泡入口被注释禁用，未实现。改为通过候选待办扫描创建。
...
1. 前后端联调：AI 对话、待办合并列表、智能回复按钮、消息下方待办创建气泡
   > ⚠️ **[实现差异]** 消息下方待办创建气泡未实现，仅实现候选待办扫描流程。
