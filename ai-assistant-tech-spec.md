...
| 8 | 邮件撰写收件人下拉选择 | [2.8.1](#sec-2.8.1) | ✅ | `/api/users` 已有 ✅ | 下拉选择器已实现 ✅ | 撰写邮件时收件人支持搜索下拉选择，自动排除当前用户 |  |
| 9 | 邮件待办点击跳转对应邮件 | [2.2](#sec-2.2) | ✅ | `emailsApi.getDetail` 已有 ✅ | 跳转逻辑已实现 ✅ | 点击邮件待办自动切换到邮件视图，选中并打开对应邮件详情 |  |
| 10 | AI 面板独立于视图切换 | [2.9](#sec-2.9) | ✅ | N/A | 已修复 ✅ | AI 面板和浮动按钮移出视图模板，切换消息/邮件视图不再影响面板状态 |  |
...
- **点击跳转** ✅：点击 pending 状态的待办项跳转到对应的原始消息或邮件
  ...
  - **邮件待办**：点击后自动跳转到对应邮件 ✅
    - AIPanel emit `jump-to-email` 携带 `email_id`
    - App.vue 接收后关闭 AI 面板 → 切换到邮件视图（`viewMode = 'emails'`）→ 调用 `emailsApi.getDetail(email_id)` 加载邮件详情 → 设置 `externalSelectedEmailId` 通知 EmailList 选中并高亮该邮件
    - EmailList watch `externalSelectedId`：自动切换到收件箱 → 加载列表 → 查找并选中对应邮件（高亮）
...
- **中栏 EmailDetail**：
  ...
  - **写邮件模式**：表单包含收件人、主题、正文类型切换（markdown/text）、正文输入框（Markdown 模式下提供实时预览）、附件选择器（从已有文件列表多选）。
    > ✅ **[v3.4 新增]** 收件人字段支持**搜索下拉选择**：聚焦/点击 ▼ 展开用户列表，输入文字实时过滤匹配联系人（按姓名或邮箱），点击列表项自动填充 `姓名 <邮箱>` 格式。自动排除当前用户。进入写邮件模式时自动加载用户列表（`usersApi.getAll()`）。
...
### <a id="sec-2.9"></a>2.9 AI 助手浮动按钮（v3.3 新增，v3.4 优化）

AI 助手面板关闭后，在内容区域内显示一个可交互的浮动按钮，支持拖动、贴边隐藏和点击唤起。

> ✅ **[v3.4 优化]** AI 面板和浮动按钮已移出视图模板（`<template v-if="viewMode">`），独立于消息/邮件视图存在。切换视图时 AI 面板状态保持不变——打开就保持打开，关闭才显示浮动球。浮动按钮的边界判断支持两种视图：消息视图用 `chatAreaWrapRef`，邮件视图用 `emailDetailWrapRef`。
...
## 13. v3.4 更新：邮件收件人下拉 / 邮件待办跳转 / AI面板独立于视图（2026-07-21）
...
- 收件人输入框改为**可搜索下拉选择器**
  - 聚焦或点击 ▼ 按钮展开用户列表下拉框
  - 输入文字实时过滤匹配的联系人（按姓名或邮箱模糊匹配）
  - 点击列表项自动填充 `姓名 <邮箱>` 格式到收件人输入框
  - 失焦延迟 200ms 关闭下拉（确保 mousedown 选择能先触发）
- 进入写邮件模式时自动调用 `usersApi.getAll()` 加载用户列表
- 自动排除当前用户（不能给自己发邮件）
- 下拉列表项：左侧姓名（深色 #1f2937）+ 右侧邮箱（灰色 #94a3b8），hover 蓝色高亮

### 13.2 邮件待办点击跳转到对应邮件

**前端**（`AIPanel.vue` + `App.vue` + `EmailList.vue`）：
- **AIPanel**：`handleTodoClick` 的邮件分支改为 emit `jump-to-email` 携带 `email_id`（不再弹出预览弹窗）
- **App.vue**：
  - 新增 `externalSelectedEmailId` ref
  - `handleJumpToEmail({ email_id })`：关闭 AI 面板 → 切换到邮件视图 → 退出写邮件模式 → 调用 `emailsApi.getDetail(email_id)` 加载邮件详情 → 设置 `externalSelectedEmailId` 通知 EmailList → 500ms 后重置（以便相同 ID 也能再次触发）
- **EmailList**：
  - 新增 `externalSelectedId` prop
  - 新增 watch：监听外部选中 ID 变化 → 自动切到收件箱 → 加载列表 → 查找并选中对应邮件（高亮）

### 13.3 AI 面板独立于视图切换

**问题**：AI 面板和浮动按钮原放在 `<template v-if="viewMode === 'messages'">` 内部，切换到邮件视图时整个 messages 模板被销毁，AI 面板状态丢失，回到浮动球状态。

**修复**（`App.vue`）：
- 将 AI 面板（`<AIPanel>`）和浮动按钮（`<div class="ai-fab">`）从 messages 模板中移出，放到与 `<template>` 同级的位置，独立于视图切换
- 给邮件视图的 EmailDetail 也加了 `emailDetailWrapRef` 包裹
- `getChatBounds()` 支持两种视图：优先用 `chatAreaWrapRef`（消息视图），其次用 `emailDetailWrapRef`（邮件视图）
- 切换视图时 AI 面板状态保持不变——打开就保持打开，关闭才显示浮动球

***

*内部IM智能助手 - 功能实现细节 v3.4 | 2026-07-21*
